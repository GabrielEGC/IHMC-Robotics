...
x=-1;dx=0;F=0;
    ...
    timtQP=toc; Firstcomp=1;
    fval;t0p=t0;vpoints=0*z(3*N+4:end);t0p=t0; nactflag=1;
%z=zeros(1,4*N+3);
tact=timtQP;tactp=0;
tmax=1;
tv=dt*[1:tmax/dt];
for i=1:100000;
    ...
    if 1/2*[x;dx]'*P*[x;dx]<c0
        ...
        contF=0;
        if (tact<t&&nactflag)
        vpoints=z(3*N+4:end);tactp=tact;xpoints=reshape(z(1:3*N+3),[3 N+1]);
        e4n=xpoints(1:3,:)-[x;dx;F]*ones(1,N+1);[~,n]=min(sum(e4n.^2));tactp=t-T*(n-1)*(1-Firstcomp);n,t;
        nactflag=0;
        end
        ...
        %vpoints=z(3*N+4:end);
        t0=t;timtQP=toc
        tact=t+timtQP;nactflag=1;Firstcomp=0;
        end
