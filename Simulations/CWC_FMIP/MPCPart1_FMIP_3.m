...
K=lqr(Apre,Bpre,Qm,Rm);

x(:,1)=[L0;r0;dr0];
x(:,1)=x(:,1)+0.05*x(:,1).*(2*rand(9,1)-1);

for i=1:N
    x(:,i+1)=x(:,i)+T*[zeta(4:6,i)+cross(x(4:6,i),zeta(1:3,i));x(7:9,i);1/m*zeta(1:3,i)+gv];%A_sys*x(:,i)+B_sys*u(:,i);
    ddx4p(:,i)=1/m*zeta(1:3,i)+gv;
end
