...
[t1,X1] = ode45(@(t,X) VHIPVarCoPAug(t,X,zf,rPd,nf,rfo,pRf,2),[0 3],x);
...
[t2,X2] = ode45(@(t,X) VHIPVarCoPAug(t,X,zf,rPd,nf,rfo,pRf,1),[0 3],x);
rsim1=X1(:,1:3)';
rsim2=X2(:,1:3)';
%%
kpP=0.5;
...
rDCM=X1(:,1:3)+(X1(:,7)*ones(1,3)).*X1(:,4:6);
uG=uSMGarcia(Tg,Zcg,zf,g);
rVRP=(rP-gv*(1./uG))';
...
plot3(rsim1(1,end),rsim1(2,end),rsim1(3,end),'ob')
...
plot3(rDCM(:,1),rDCM(:,2),rDCM(:,3));
plot3(rVRP(:,1),rVRP(:,2),rVRP(:,3),'c');
...
legend('Initial CoM','CoM','Final CoM','CoP','Initial CBP','CBP','DCM','VRP')
axis([-0.45 0.45 -0.7 -0.2 -0.2 1.8])
