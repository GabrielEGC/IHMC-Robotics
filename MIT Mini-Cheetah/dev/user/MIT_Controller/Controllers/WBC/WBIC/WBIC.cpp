...
void WBIC<T>::_SetEqualityConstraint(const DVec<T>& qddot) {
  if (_dim_rf > 0) {
    ...
    _dyn_CE.block(0, _dim_floating, _dim_eq_cstr, _dim_rf) = -WB::Sv_ * _JcFBD.transpose();
    _dyn_ce0 = -WB::Sv_ * (WB::A_ * qddot + WB::cori_ + WB::grav_ - _JcFBD.transpose() * _Fr_des);
  } else {
    _dyn_CE.block(0, 0, _dim_eq_cstr, _dim_floating) = WB::A_.block(0, 0, _dim_floating, _dim_floating);
...
void WBIC<T>::_ContactBuilding() {
  ...
  _JcFBD.block(0, 0, dim_accumul_rf, WB::num_qdot_) = JcFBD;
  ...
  for (size_t i(1); i < (*_contact_list).size(); ++i) {
    ...
    _JcFBD.block(dim_accumul_rf, 0, dim_new_rf, WB::num_qdot_) = JcFBD;
  ...
  //pretty_print(_JcFBD, std::cout, "[WBIC] JcFBC");
...
void WBIC<T>::_GetSolution(const DVec<T>& qddot, DVec<T>& cmd) {
  DVec<T> tot_tau;
  if (_dim_rf > 0) {
    _data->_Fr = DVec<T>(_dim_rf);// get Reaction forces
    for (size_t i(0); i < _dim_rf; ++i)
      _data->_Fr[i] = z[i + _dim_floating] + _Fr_des[i];
    tot_tau = WB::A_ * qddot + WB::cori_ + WB::grav_ - _JcFBD.transpose() * _data->_Fr;
  } else {
...
void WBIC<T>::_SetOptimizationSize() {
  ...
  if (_dim_rf > 0) {
    ...
    _JcFBD = DMat<T>(_dim_rf, WB::num_qdot_);
