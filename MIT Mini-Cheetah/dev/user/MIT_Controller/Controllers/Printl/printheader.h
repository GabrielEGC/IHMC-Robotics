...
template <class T>
void print_arraytr(T* array, u16 rows, u16 cols)
{
    for(u16 r = 0; r < rows; r++)
    {
        for(u16 c = 0; c < cols; c++)
            std::cout<<(fpt)array[c*rows+r]<<" ";
        printf("\n");
    }
}
...
template <class T>
void print_named_arraytr(const char* name, T* array, u16 rows, u16 cols)
{
    printf("%s:\n",name);
    print_arraytr(array,rows,cols);
}
...
void print_update_data(T* update, s16 horizon)
{
  ...
  print_named_array("uweights: ",update->uweights,1,3);
  print_named_array("desired trajectory: \n",update->traj,horizon,13);
  print_named_arraytr("Gait",update->gait,4,horizon); //print_named_array("gait",update->gait,horizon,4);
}