...
void WBC_Ctrl<T>::_ComputeWBC() {
  _kin_wbc->FindConfiguration(_full_config, _task_list, _contact_list, _des_jpos, _des_jvel);// TEST
  _wbic->UpdateSetting(_A, _Ainv, _coriolis, _grav);// WBIC
...
void WBC_Ctrl<T>::run(void* input, ControlFSMData<T> & data){
  ++_iter;
  _UpdateModel(data._stateEstimator->getResult(), data._legController->datas);  // Update Model
  _ContactTaskUpdate(input, data);// Task & Contact Update
  _ComputeWBC();// WBC Computation
  // TEST
  //T dt(0.002);
  ...
  //_ComputeWBC();
  // END of TEST
  _UpdateLegCMD(data);// Update Leg Command
  _LCM_PublishData(); // LCM publish
}
