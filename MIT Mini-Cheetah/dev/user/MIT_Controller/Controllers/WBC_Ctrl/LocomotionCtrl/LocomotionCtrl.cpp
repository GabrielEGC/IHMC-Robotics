...
#include <WBC_Ctrl/ContactSet/SlidingContact.hpp>
...
#include <WBC_Ctrl/TaskSet/LinkSlidTask.hpp>

template<typename T>
LocomotionCtrl<T>::LocomotionCtrl(FloatingBaseModel<T> model):WBC_Ctrl<T>(model){
  _body_pos_task = new BodyPosTask<T>(&(WBCtrl::_model)); //study 
  ...
  _fosl_contact[0] = new SlidingContact<T>(&(WBCtrl::_model), linkID::FR);
  _fosl_contact[1] = new SlidingContact<T>(&(WBCtrl::_model), linkID::FL);
  _fosl_contact[2] = new SlidingContact<T>(&(WBCtrl::_model), linkID::HR);
  _fosl_contact[3] = new SlidingContact<T>(&(WBCtrl::_model), linkID::HL);
  _fosl_task[0] = new LinkSlidTask<T>(&(WBCtrl::_model), linkID::FR);
  _fosl_task[1] = new LinkSlidTask<T>(&(WBCtrl::_model), linkID::FL);
  _fosl_task[2] = new LinkSlidTask<T>(&(WBCtrl::_model), linkID::HR);
  _fosl_task[3] = new LinkSlidTask<T>(&(WBCtrl::_model), linkID::HL);
}
template<typename T>
...
void LocomotionCtrl<T>::_ContactTaskUpdate(void* input, ControlFSMData<T> & data){
  ...
  /*for(int i=0; i<4; ++i){pretty_print((_input_data->Fr_des[i]), std::cout, "Fr in input inside _ContactTaskUpdate");}*/
  _ParameterSetup(data.userParameters);// Wash out the previous setup
  _CleanUp();
  ...
  for(size_t leg(0); leg<4; ++leg){
    if(_input_data->is_slidcntct[leg] < 0.5){
      ...
      _fosl_contact[leg]->setMu((T)_input_data->muv[leg]);
      _fosl_contact[leg]->setRpla(_input_data->Rpla[leg]);//(Eigen::Matrix<T,3,3>)
      _fosl_contact[leg]->setRFDesired((DVec<T>)(_input_data->Fr_des[leg]));//VERIFYYYYYYYYY
      _fosl_contact[leg]->UpdateContactSpec();
      _fosl_task[leg]->setRpla(_input_data->Rpla[leg]);//(Eigen::Matrix<T,3,3>)
      _fosl_task[leg]->UpdateTask(&(_input_data->pFoot_des[leg]), _input_data->vFoot_des[leg], _input_data->aFoot_des[leg]);//zero_vec3);
      WBCtrl::_contact_list.push_back(_fosl_contact[leg]);
      WBCtrl::_task_list.push_back(_fosl_task[leg]);
    }
  }
}
  ...
  for(size_t leg(0); leg<4; ++leg){
    if(_input_data->contact_state[leg] > 0.){ // Contact
      ...
      _foot_task[leg]->UpdateTask(
          ...
          _input_data->aFoot_des[leg]);//zero_vec3);
      WBCtrl::_task_list.push_back(_foot_task[leg]);
    }
  }
}
  ...
  for(size_t leg(0); leg<4; ++leg){
    ...
    if(_input_data->is_slidcntct[leg] < 0.5){
      if(_input_data->contact_state[leg]>0.){
        for(size_t i(0); i<3; ++i){
          _Fr_result[leg][i] = WBCtrl::_wbic_data->_Fr[iter];
          ++iter;
        }
      }
    }
    else{//if(_input_data->contact_state[leg]>0.){//prolly 1
      _Fr_result[leg][2] = WBCtrl::_wbic_data->_Fr[iter];//verify//Rot
      ++iter;
    }
    if(_input_data->contact_state[leg] > 0.){ // Contact
