    ...
    : FSM_State<T>(_controlFSMData, FSM_StateName::LOCOMOTION, "LOCOMOTION")
{
  if(_controlFSMData->_quadruped->_robotType == RobotType::MINI_CHEETAH){
    cMPCOld = new ConvexMPCLocomotion(_controlFSMData->controlParameters->controller_dt,
        ...
        27 / (1000. * _controlFSMData->controlParameters->controller_dt),//dt=0.002, itbet=13
...
FSM_StateName FSM_State_Locomotion<T>::checkTransition() {
  ...
  if(locomotionSafe()) {
    ...
    this->nextStateName = FSM_StateName::PASSIVE;//RECOVERY_STAND
    this->transitionDuration = 0.;
    rc_control.mode = RC_mode::OFF;//RECOVERY_STAND
  }
...
bool FSM_State_Locomotion<T>::locomotionSafe() {
  ...
  const T max_roll = 40;//40
  const T max_pitch = 50;
  if (cMPCOld->chSfOri==true){//optimizeeeeeeee
...
void FSM_State_Locomotion<T>::LocomotionControlStep() {
  ...
  this->checkSafeOrientation = cMPCOld->chSfOri;//optimizeeeeeeee
  if(this->_data->userParameters->use_wbc > 0.9){
    ...
    //cout<<"pBody_des: "<<cMPCOld->pBody_des<<endl;
    //cout<<"pBody_RPY_des: "<<cMPCOld->pBody_RPY_des<<endl;
    
    for(size_t i(0); i<4; ++i){
      //cout<<"\npFoot_des"<<cMPCOld->pFoot_des[i]<<endl;
      //cout<<"\nvFoot_des"<<cMPCOld->vFoot_des[i]<<endl;
      //cout<<"\naFoot_des"<<cMPCOld->aFoot_des[i]<<endl;
    ...
    _wbc_data->is_slidcntct = cMPCOld->is_slidcntct;

    _wbc_ctrl->run(_wbc_data, *this->_data);
    for(int leg(0); leg<4; ++leg){//Useful? Why no in the WBC
      ...
      //pretty_print(vDes_backup[leg], std::cout, "vDes_backup");
    }
  }
}
/**
