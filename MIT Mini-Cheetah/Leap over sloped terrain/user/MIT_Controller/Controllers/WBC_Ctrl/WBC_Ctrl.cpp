...
void WBC_Ctrl<T>::_ComputeWBC() {
  ...
  // WBIC
  //pretty_print(_wbic->_Fr_des, std::cout, "Fr from to wbic before UpdateSetting");//already screwed
  //_wbic->Rpla = Rpla;//???
  _wbic->UpdateSetting(_A, _Ainv, _coriolis, _grav);
  //pretty_print(_wbic->_Fr_des, std::cout, "Fr from to wbic before MakeTorque");
  _wbic->MakeTorque(_tau_ff, _wbic_data);
  //pretty_print(_wbic->_Fr_des, std::cout, "Fr from to wbic after MakeTorque");
}
...
void WBC_Ctrl<T>::run(void* input, ControlFSMData<T> & data){
  ++_iter;
      /*for(int i=0; i<4; ++i){
      pretty_print((input->Fr_des[i]), std::cout, "Fr in input insider run");
    }*/
  //pretty_print(_wbic->_Fr_des, std::cout, "Fr from to wbic before _UpdateModel");
  // Update Model
  _UpdateModel(data._stateEstimator->getResult(), data._legController->datas);
  //pretty_print(_wbic->_Fr_des, std::cout, "Fr from to wbic before _ContactTaskUpdate");
  // Task & Contact Update
  _ContactTaskUpdate(input, data);
  // WBC Computation
  //pretty_print(_wbic->_Fr_des, std::cout, "Fr from to wbic before _ComputeWBC");
  _ComputeWBC();
  // TEST
  //T dt(0.002);
