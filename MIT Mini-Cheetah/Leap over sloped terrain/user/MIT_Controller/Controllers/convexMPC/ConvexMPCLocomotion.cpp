...
void ConvexMPCLocomotion::run(ControlFSMData<float>& data) {
  ...
  for(int i = 0; i < 4; i++){
    ...
    if(gait != &jumping){
      ...
      footSwingTrajectories[i].setHeight(seResult.position[2]-heightoffCoM_prev-0.2);
    ...
    if (gait!=&jumping){
      ...
      Pf[2] = apla2(0)+apla2(1)*Pf[0]+apla2(2)*Pf[1]-0.003;//JUMPINGUNC
    }
    //Pf[2] = 0.0;
  ...
  updateMPCIfNeeded(gait, mpcTable, data, omniMode, fmax(gait->getHoriz()-jump_state.cshift,nxtgait->getHoriz()));
...
void ConvexMPCLocomotion::updateMPCIfNeeded(Gait* gait, int *mpcTable, ControlFSMData<float> &data, bool omniMode, int horLength) {
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0){
    ...
    if(current_gait == 4 || firstRun){
      ...
      if(gait==&jumping){
        /*heightoffCoM=apla2(0)+apla2(1)*(world_position_desired[0] + dtMPC * horLength * v_des_world[0])+apla2(2)*(world_position_desired[1] + dtMPC * horLength * v_des_world[1]);
        _body_height = heightoffCoM + (float)data.userParameters->bo_height; // JUMPINGUNCOM*/
        if(data.userParameters->ori_pla_body<0.5){quatdes=rpyToQuat(orienvecdes);}
        else{quatdes=rotationMatrixToQuaternion(rpyToRotMat(orienvecdes)*Rpla2[0].transpose());}
        
        Vec3<float> v_des_robot_j((float)data.userParameters->dxjump, (float)data.userParameters->dyjump, 0);
        Vec3<float> v_des_world_j = seResult.rBody.transpose() * v_des_robot_j;//omniMode ? v_des_robot_j :
        v_des_world = v_des_world_j;
      }else{
        if(data.userParameters->ori_pla_body<0.5){quatdes=rpyToQuat(orienvecdes);}
        else{quatdes=rotationMatrixToQuaternion(rpyToRotMat(orienvecdes)*Rpla[0].transpose());}
      }
      ...
      update_traj(trajAll);//std::cout<<"trajAll: "<< *trajAll<<std::endl;
      Matrix<float,4,1> quatdes;
